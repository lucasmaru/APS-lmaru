...
# -*- coding: utf-8 -*-
"""
Created on Sat Jun  7 15:43:59 2025
...
GRAFICAR = True  # Cambiar a False si no se desea mostrar gráficos
plt.close('all')
#%%ECG
...
#%% BUSQUEDA DE ORDEN MÍNIMO PARA FIR - REMEZ

# import time

# def encontrar_filtro_remez(f_stop, f_pass, fs, orden_inicial=5002, orden_max=25002, paso=2000):
#     print("Iniciando barrido de orden para diseño con remez...\n")
#     for cant_coef in range(orden_inicial, orden_max + 1, paso):
#         orden = cant_coef - 1
#         try:
#             print(f"Probando orden {orden}...", end=' ')
#             # Normalización de bandas
#             # bands = [0, 0.1, 1 , 30, 50, 500]
#             # desired = [0, 1, 0]
#             bands = [0, f_stop[0],  # Banda 1 (rechazo)
#             f_stop[0], f_pass[0],  # Transición (sin peso)
#             f_pass[0], f_pass[1],  # Banda 2 (paso)
#             f_pass[1], f_stop[1],  # Transición (sin peso)
#             f_stop[1], fs/2]       # Banda 3 (rechazo)
#             desired = [0, 0, 1, 1, 0]  # Una ganancia por cada par de bandas

#             start = time.time()
#             fir = sig.remez(numtaps=cant_coef, bands=bands, desired=desired , fs=fs)
#             duracion = time.time() - start

#             if np.allclose(fir, 0):
#                 print("❌ Resultado nulo.")
#                 continue
#             else:
#                 print(f"✅ Convergió en {duracion:.2f} s (orden {orden})")
#                 return fir, orden
...
#     print("⚠️ No se encontró un orden que funcione en el rango dado.")
#     return None, None

# # Ejecutamos barrido
# mi_fir_2, orden_fir_2 = encontrar_filtro_remez(f_stop, f_pass, fs)
